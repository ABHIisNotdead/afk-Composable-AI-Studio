...
function FlowEditor() {
  ...
  const [outputTab, setOutputTab] = useState<'console' | 'text' | 'images'>('console');
...
const onDrop = useCallback((event: React.DragEvent) => {
  ...
  let nodeData = { label: label, type: type, prompt: "" };
  
  if (savedDataString) {
    nodeData = JSON.parse(savedDataString);
  } else {
    // Default system prompts for specialized config nodes
    if (type === 'prog-lang') nodeData.prompt = "HTML/Tailwind"; 
    if (type === 'database') nodeData.prompt = "LocalStorage"; 
    if (type === 'image-gen') nodeData.prompt = "Photorealistic, 4k, cinematic lighting";
    if (type === 'ai-worker') nodeData.prompt = "Analyze the input data";
  }
  ...
  // 3. Specialized Modular Logic
  if (type === 'start') {
    // Pure Entry Point - Simplified Start
    nodeStyle = { 
      backgroundColor: '#10b981', 
      color: 'white', 
      border: '4px solid #059669', 
  ...
  } else if (type === 'prog-lang') {
    // Modular Orange Coding Node
    nodeStyle = { 
  ...
  const newNode = { 
      id: `${type}-${Date.now()}`, 
      // Start node remains 'input' type (only has an outgoing handle)
  ...
  return (
    ...
    <div className={`w-2 h-2 rounded-full ${
        JSON.stringify({nodes, edges}) === localStorage.getItem(`build-hash-${projectId}`) 
...
{isChatOpen && (
        ...
        <div className="flex-1 h-full relative flex flex-col min-w-0" ref={wrapperRef}>
          <div className="flex-1 relative">
           ...
           <ReactFlow
              ...
              <CodeEditor filename={activeFile} code={files[activeFile] || ""} onChange={(c: string) => setFiles({...files, [activeFile]: c})} onClose={() => setViewMode('graph')} />
            )}
          </div>
  ...
  <div className="flex flex-col border-t border-slate-800 bg-slate-950">
    ...
    <div className="flex items-center px-4 h-10 border-b border-slate-900 bg-slate-900/50 gap-4 shrink-0">
      {['console', 'text', 'images'].map((tab) => (
        <button
          key={tab}
          onClick={() => setOutputTab(tab as any)}
          className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${
            outputTab === tab ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'
          }`}
        >
          {tab}
        </button>
  ...
  isOpen={isOutputOpen} // Ensure this is the correct state variable
  logs={logs}/>}
